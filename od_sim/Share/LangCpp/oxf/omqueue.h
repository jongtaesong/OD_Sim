...
protected :
    ...
    inline virtual Concept& getCurrent(void * pos) const {
        ...
        if ( (m_head < m_tail) &&
             ((reinterpret_cast<Rhp_int64_t>(pos) < static_cast<Rhp_int64_t>(m_head)) || 
              (reinterpret_cast<Rhp_int64_t>(pos) >= static_cast<Rhp_int64_t>(m_tail))) ) {
        	// out of range (before head or after tail)
           	return OMNullValue<Concept>::get();
        } else {
        	if ( (reinterpret_cast<Rhp_int64_t>(pos) < static_cast<Rhp_int64_t>(m_head)) &&
        	     (reinterpret_cast<Rhp_int64_t>(pos) >= static_cast<Rhp_int64_t>(m_tail)) ) {
        		// out of range (between head and tail)
               	return OMNullValue<Concept>::get();
            }
        }   
        return m_myQueue.getAt(static_cast<Rhp_int64_t>(reinterpret_cast<Rhp_int64_t>(pos)));
        //#]
    }
    ...
    inline virtual void getNext(void*& pos) const {
        //#[ operation getNext(void*& ) const
        Rhp_int32_t i = reinterpret_cast<Rhp_int64_t>(pos);
        
        ++i;
        if (i == static_cast<Rhp_int64_t>(m_myQueue.getSize())) {
        	i = 0;
        }
        if (i == m_head) {
        	i = static_cast<Rhp_int64_t>(m_myQueue.getSize());
        }
